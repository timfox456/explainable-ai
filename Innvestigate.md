...
Sensitivty means that if we change the inputs (i.e., the pixels on an image), and the output classification changes as a result, those same pixels
...
The need is there to make a common interface and standard for various implementations of explainable methods, and bring them to gether in a common
interface.  iNNvestigate provides a common python library to perfomr this.
...
to combined the Implementation Invariance of backprogagation methods with the Sensitity of LRMs,etc

**Deconvnet**: [DeConvNet](https://arxiv.org/pdf/1505.04366.pdf) is a network that provides a *deconvlutional* behavior for CNNs, also 
...
iNNvestigate provides a standard interface and python framework to impolmenent an umber of different analyzers for showing attribution of the results of 
neural network predictionns. By using iNNvestigate, one can quickly run a number of different approaches for explainable AI (XML).
